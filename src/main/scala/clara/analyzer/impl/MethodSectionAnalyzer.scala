...
// case class MethodSectionAnalyzer(parentEnv: Env) {
...
//     val typeAn = TypeExprAnalyzer(currentState.env).walkTypeExpr(typeExpr)
...
//       An.step(methodAsts)(WalkDeclState.begin(parentEnv)){ case (currentState, methodAst) =>
//         (methodAst match {
...
//     walkTargetTypeName(targetTypeName).flatMap { case (targetType, uniq) =>
//       PatternAnalyzer(parentEnv, parentEnv).walkAssignment(selfPattern, targetType).map((targetType, uniq, _))
...
//     parentEnv.useType(name, pos).flatMap(walkTargetType(pos))
//   }
