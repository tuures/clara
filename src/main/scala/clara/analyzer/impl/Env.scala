...
case class Env(types: Namespace[Typ], values: Namespace[MonoType], methods: TypeInfo[Terms.MethodSection]) {
...
object Env {
  def empty = Env(Namespace.empty[Typ], Namespace.empty[MonoType], TypeInfo.empty[Terms.MethodSection])
}
