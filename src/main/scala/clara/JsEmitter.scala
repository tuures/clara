...
object JsEmitter {
  ...
  object Impl {
    ...
    def walkValueExpr(indent: Indent)(valueExpr: ValueExpr): String = valueExpr match {
      ...
      case MemberSelection(e, member, _, _, _) => safe"${walkValueExpr(indent)(e)}.$member"
      case Call(callee, argument, _) => safe"${walkValueExpr(indent)(callee)}(${walkValueExpr(indent)(argument)})"
    }
